canvas.app = new(function(app) 
{
    // Creates a new canvas module and appends it as a child
    ...
    this.resolution = function ( app )
    {
        ...
        this.width = app.resolution.width;
        this.height = app.resolution.height;
        this.compositeScale = compositeScale = (app.resolution.width+app.resolution.height)/(app.width+app.height);
        
        resX = app.resolution.width/app.width;
        resY = app.resolution.height/app.height;
    }

    //Private
    var compositeScale = this.compositeScale;
    var resX = app.resolution.width/app.width;
    var resY = app.resolution.height/app.height;

    var controls, zmesh, geometry;
    ...
    streams.addon.timeframe.invoke = function () {
        calc();
        render();
        compute();
    }
    ...
    function setupCameraBindings (stream) {
        ctx.streams.addon.binding(stream, [
    ...
    //All stream lengths have to match <> !!!streams.length!!! because of parallel stream
    function setupUniformBindings (stream) {
        ...
        ctx.scene.farlight.uniforms.alphas = ctx.streams.addon.binding(stream, [
        ...
        streams.length,
        true);
        streams.addon.buffer.eval(stream, [[ctx.scene.farlight.uniforms.alphas[0]], [['darklayer', -700]], [['linear', 500]]], true);
    }
        ...
        loader.load(
            ...
            function ( texture ) {
                ...
                window.addEventListener('keyup', function (e) {
                        streams.addon.timeframe.keyPauseToggle(e);
                    }
                );
                streams.addon.timeframe.run('timeline');
            },
