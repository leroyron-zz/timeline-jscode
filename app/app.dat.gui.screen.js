window.app = {
    stream: {
        ...
        timeframe: {
            running: true,
            lapse: 10  // lapse is good while recording accuracy
        },
        buffer: {
            evals: 0
        }
...
(function (THREE, app, dat, Stats) {
    const canvases = []
    // init App inside of a container
    window.init = function (parent, child) {
        const container = document.getElementById(parent)
        let info = document.getElementById(child)// Added info for app output(Remove at will)
        window.stats = new Stats()// Added stats for app output(Remove at will)
        const stats = window.stats
        stats.showPanel(0) // 0: fps, 1: ms, 2: mb, 3+: custom
        info.appendChild(stats.dom)// Added stats for app output(Remove at will)
        window.createCanvas = function (option) {
            var canvas = {}
            if (option !== '3d') {
                ...
                canvas.renderer.setSize(app.resolution.width,
    ...
    const onresize = function onresize () {
        ...
        if (canvases.length > 0) {  // Resize all canvases
            for (var cvi = 0, cvslen = canvases.length; cvi < cvslen; cvi++) {
                ...
                canvases[cvi].app.resolution(app)
            }
        }
        responsive()
    }
    ...
    const responsive = function () {
        window.storyboard = (app.width - 375) + 'px'
    }
    ...
    const exitFullscreen = function (element) {
    ...
    const toggleFullscreen = function (bool, elem) {
        if (!bool) {
            exitFullscreen()
        } else {
    ...
    // Resolutions
    gui.appResolution = gui.controls.recording.__folders.Adjustments.add(app, 'resolution',
        {
            auto: [1280, 720],
            r2160p: [3840, 2160],
            r1440p: [2560, 1440],
    ...
    gui.appResolution.setValue(app.resolution)// Set default
    gui.remember(gui.appResolution)

    // MotionBlur
    gui.controls.recording.addFolder('MotionBlur')
    gui.controls.recording.__folders.MotionBlur.add(app.motionblur, 'shutterspeed', 0, 100)
    gui.controls.recording.__folders.MotionBlur.add(app.motionblur, 'influence', 0, 100)

    // Uniforms
    gui.controls.recording.addFolder('Uniforms')
    gui.controls.recording.__folders.Uniforms.add(app.uniforms, 'godray_amount', 0, 100)
    gui.controls.recording.__folders.Uniforms.add(app.uniforms, 'screenshadow_alpha', 0, 100)
    gui.controls.recording.__folders.Uniforms.add(app.uniforms, 'ash_direction', 0, 9)
    // //RECORDING////

    // //STORYBOARD////
    gui.appSBDuration = gui.controls.storyboard.add(app.storyboard, 'duration')
    gui.appSBSegment = gui.controls.storyboard.add(app.storyboard, 'segment')
    gui.appSBActionETA = gui.controls.storyboard.add(app.storyboard, 'actionETA')
    ...
    gui.appSArg0 = gui.controls.stream.__folders.arguments.add(app.stream.access, 'continuance')
    gui.appSArg1 = gui.controls.stream.__folders.arguments.add(app.stream.access, 'skip')
    gui.appSArg2 = gui.controls.stream.__folders.arguments.add(app.stream.access, 'tCount')
    gui.appSArg3 = gui.controls.stream.__folders.arguments.add(app.stream.access, 'revert')
    gui.appSArg4 = gui.controls.stream.__folders.arguments.add(app.stream.access, 'mCount')
    gui.appSArg5 = gui.controls.stream.__folders.arguments.add(app.stream.access, 'leap')
    gui.appSArg6 = gui.controls.stream.__folders.arguments.add(app.stream.access, 'reset')

    gui.controls.stream.addFolder('timeframe')
    gui.appSTimeframeRunning = gui.controls.stream.__folders.timeframe.add(app.stream.timeframe, 'running')
    gui.appSTimeframeLapse = gui.controls.stream.__folders.timeframe.add(app.stream.timeframe, 'lapse')

    gui.controls.stream.addFolder('runtime')
    gui.appSRuntimeThrust = gui.controls.stream.__folders.runtime.add(app.stream.runtime, 'thrust')
    gui.appSRuntimeMeasure = gui.controls.stream.__folders.runtime.add(app.stream.runtime, 'measure')
    gui.controls.stream.addFolder('buffer')

    gui.appSBufferEvals = gui.controls.stream.__folders.buffer.add(app.stream.buffer, 'evals')
    ...
    // FullScreen
    gui.appFullscreen = gui.controls.settings.add(app, 'fullscreen')
    gui.appFullscreen.onFinishChange(function (value) {
        toggleFullscreen(value, document.documentElement)
    })
    gui.appFullscreen.setValue(app.fullscreen)// Set default

    // Touch
    gui.appTouch = gui.controls.settings.add(app, 'touch')
    gui.appTouch.onFinishChange(function (value) {

    })
    gui.appTouch.setValue(app.touch)// Set default

    // Sound
    gui.appSound = gui.controls.settings.add(app, 'sound')
    gui.appSound.onFinishChange(function (value) {

    })
    gui.appSound.setValue(app.sound)// Set default

    // Charts//charts:'gifImage',
    gui.fullScreenEventHandler = 'fullscreenchange'
    if (typeof document.mozFullScreenElement !== 'undefined') {
        gui.fullScreenEventHandler = 'mozfullscreenchange'
    } else if (typeof document.webkitFullscreenElement !== 'undefined') {
        gui.fullScreenEventHandler = 'webkitFullscreenElement'
    }
...
})(window.THREE, window.app, window.dat, window.Stats)
