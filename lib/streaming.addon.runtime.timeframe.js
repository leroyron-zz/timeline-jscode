...
(function (Streaming) {
    ...
    that.revertCallbacks = function (revertPos) {
        for (var rr = 0; rr < this.rrclen; rr++) {
            this.revertCalls[rr][1](this.revertCalls[rr][0], revertPos)
        }
    }
    // hardwire reverting to timeframe scope
    ...
    that.update = function () {
        ...
        // timeframeThurstingStreamingUtilizationAsRuntimeSumingValues()
    }
    that.updateCalls = []
    that.uclen = 0
    ...
    var timeframeThurstingStreamingUtilizationAsRuntimeSumingValues = function () {
        _runtime.access.output_utilizeThrustData = function (value, node, property) {
            // debugger;
            var setBind = this.bindings.ids['_bi' + node]
            var setBindProperty = setBind[property]
