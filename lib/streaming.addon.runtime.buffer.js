...
(function (Streaming) {
    ...
    var valueOffset
    var deltaSetOffset
    that.eval = function (stream, instructions, relative, precision) {
        buffOffset = 0
    ...
    that.zeroOut = function (stream, from, to) {
        ...
        var propDataLength = state == 'prebuff'
        ? _runtime[stream].propDataLength + 1
        : _runtime[stream].length + 1

        var continuancePosValData0 = state == 'prebuff'
        ? 1
        : _runtime[stream].continuancePosValData0
    ...
    var buff = function (stream, node, prop, evalData, relative) {
        ...
        var byteOffset = state == 'prebuff'
        ? _runtime[stream].data[data0PositionI]
        : _runtime.access.data[data0PositionI]
