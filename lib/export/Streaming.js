    ...
    Class.addon.binding = function () {
        ...
        var _execThen = function (val, ease, duration, leapCallback, reassign, dispose, zeroIn, skipLeap) {
            if (val && ease && duration) {
                if (this.then.caller) {
                    this.execNode = Class.addon.runtime.access.bindings.ids[this.binding].node || this.execNode
                    this.execBinding = this.then.caller ? Class.addon.runtime.access.bindings.ids[this.binding][this.then.caller.binding].binding : this.execBinding
        ...
        var _execWait = function (val, ease, duration, leapCallback, reassign, dispose, zeroIn, skipLeap) {
            if (val) {
                _buffer.valIn('timeline', [this.execNode], [this.execBinding], this.execLastVal, this.execLastOffset, this.execLastOffset+val, undefined, undefined, leapCallback, reassign, dispose, zeroIn, typeof skipLeap == 'undefined' ? true : skipLeap)
                this.execLastOffset += val
            }
            return this
        }
        return that
