        ...
        this.open = this.open || function () {
            ...
            this.access = this.access || function (setcontinuance, setskip, setrCount, settCount, setrevert, setmCount, setleap, setreset) {
                ...
                this._resetLeap = function () {
                    setBind = this.bindings.ids['_bi' + nodeBsIK]
                ...
                this._readAll = function (rCount) {
                    for (
                        ...
                        if (nodeBsIK == -1) {
                            sI = endPosI
                            this._updateDataPos()
                        } else if (dataPos + rCount > propDataLength - 1) {
                            if (!revert) {

                            } else {
                                if (nodeBsIK != 101) this._checkOutRevert(rCount)
                                else this._checkOutRevertCallback(rCount)
                            }
                            this._resetLeap()
                        } else if (rCount > 0) {
                            // Data Level//
                ...
                this._thrustAll = function (tCount) {
                    for (
                        ...
                        if (nodeBsIK == -1) {
                            sI = endPosI
                            this._updateDataPos()
                        } else if (dataPos + tCount > propDataLength - 1) {
                            if (!revert) {

                            } else {
                                if (nodeBsIK != 101)
                                    this._checkOutRevert(tCount)
                                else
                                    this._checkOutRevertCallback(tCount)
                            }
                            this._resetLeap()
                        } else if (tCount > 0) {
                            // Data Level//
                ...
                this._measureAll = function (mCount) {
                    ...
                    for (
                        ...
                        if (nodeBsIK == -1) {
                            sI = endPosI
                            this._updateDataPos()
                        } else if (dataPos + mCount > propDataLength - 1) {
                            if (!revert) {

                            } else {
                                if (nodeBsIK != 101)
                                    {this._checkOutRevert(mCount)}
                                else
                                    {this._checkOutRevertCallback(mCount)}
                            }
                            this._resetLeap()
                        } else if (mCount > 0) {
                            // Data Level//
                            ...
                            setBindProperty = setBind[propBsIK]
                            leapPos = setBind.node[stream][setBindProperty.binding] ? setBind.node[stream][setBindProperty.binding].leapNext : setBind.node[stream][setBindProperty.property].leapNext
                            this.output_utilizeMeasureData(this._dataReturn(mCount), nodeBsIK, propBsIK)
                        } else {
                            return
                        }
                    }
                ...
                this._syncOffsets = function (syncI) {
                    ...
                    for (
                        ...
                        if ((sI - reads) % data0PropDataLength == partFrac) {
                            ...
                            let setBind = this.bindings.ids['_bi' + nodeBsIK]
                            let setBindProperty = setBind[propBsIK]
                            let shift = setBind.node[this.stream][setBindProperty.binding]._shift
...
function (Streaming) {
    ...
    Class.addon.binding = function () {
        ...
        var _assignExec = function (This, who, _called) {
            ...
            This.array = This.array || new Int32Array(new ArrayBuffer(_runtime.access.propDataLength * 4))
            ...
            if (duration) {
                ...
                if (duration >= _runtime.access.propDataLength) {
                    ...
                    if (end > endDataPosI) {
                        ...
                        if (leapCallback) _buffer.assignLeap('timeline', [This.execNode], [This.execBinding], true, end - firstDataPosI, leapCallback, reassign, dispose, zeroIn, true)
                    } else {
                        zeroIn = _runtime.access.data[end]
                        This.array = _runtime.access.data.slice(This.execDataPosI, end)
                        _execFlood(This.value, This.execDataPosI, end)
                        if (leapCallback) _buffer.assignLeap('timeline', [This.execNode], [This.execBinding], true, end - firstDataPosI, leapCallback, reassign, dispose, zeroIn)
                    }
                }
        ...
        var _execAt = function (at, leapCallback, reassign, dispose, zeroIn, skipLeap, This) {
            ...
            if (leapCallback) _buffer.assignLeap('timeline', [This.execNode], [This.execBinding], true, at, leapCallback, reassign, dispose, zeroIn, true)
            This.value = zeroIn / This.precision
            // }
            return This
        }
