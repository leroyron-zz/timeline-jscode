...
var Check=function(c,a){for(let b=0;b<a.length;b++)if(c==a[b])return!1;return!0},Optin=function(c,a){var b=[],d;for(d in c)Check(d,a)&&b.push(d);for(d=0;d<b.length;d++)delete c[b[d]];return c};
...
scale:a.scale,alpha:a.alpha,sprite:a.sprite}});return a};this.ctx.inView=function(c){return 1>=c&&-1<=c};this.ctx.coords=function(c){for(let a=["x","y","width","height"],b=0;b<a.length;b++)c[a[b]]&&(c[a[b]]=isNaN(c[a[b]])?/\d+/gi.exec(c[a[b]])<<0:this.inView(c[a[b]])||"x"!=a[b]&&"width"!=a[b]?this.inView(c[a[b]])||"y"!=a[b]&&"height"!=a[b]?c[a[b]]:2*-(c[a[b]]/this.canvas.height)+1:c[a[b]]/this.canvas.width*2-1);return c};this.ctx.coords.prototype.viewport=function(c){c.x=2*c.x-1;c.y=2*-c.y+1};
this.ctx.coords.prototype.normal=function(c){c.x=c.x/2+.5;c.y=-c.y/2+.5};this.ctx.loadScript=function(c,a){window.script=document.createElement("script");var b=window.script;b.src=c;b.callback=a;b.onload=function(){this.callback&&this.callback()};document.body.appendChild(b)};