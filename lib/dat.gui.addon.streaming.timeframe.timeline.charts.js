/*
dat.gui.streaming.timeline AddOn: charts
...
(function (gui) {
    ...
    {
        var seeksegPos = (_timeline.sliderPos-_timeline.prevSegmentPos)/(_timeline.segmentPos-_timeline.prevSegmentPos)*100;
    ...
    var randFromToString = function(nfrom, nto) {//To fake the values
        ...
        return ranNum = ranNum < 0 ? ranNum: '+'+ranNum;
    };
    genBlocks(divNodes);
    genBlocks(100, 'tri');
    ...
    function genBlocks(gen, genName) {
        ...
        for (var n=0; n<nlen; n++) 
        {
            ...
            var divBuffers = document.createElement('div');
            ...
            for (var p=0; p<prop.length; p++) {
                ...
                for (var s=0; s<prop[p][1].length; s++) {
                    ...
                    var segment4length = 6200;//gui.nextSegmentPos - gui.segmentPos;//At segment 4 22000 - 15800
                    
                    if (isNum) {
                        var sumCheck = 0;
                        var fNum = 0;
                        prop[p][1][s] = [prop[p][1][s][0]];
                        for (var fs=segment4length;fs>0; fs-=fNum) {//Randomize fake data
                    ...
                    for (var b=1; b<prop[p][1][s].length; b++) {
    ...
    gui.addon.onupdate( divSeekseg, _timeline, _onupdate );

    gui.addon.onpass( divCsegment, _timeline, _onpass );

    //GUI
    gui.controls.charts = gui.addFolder('Charts');
    var chartsOpen = false;
    openToggleCharts = function() {
        if (chartsOpen) {
            divCharts.style.display = 'none';
            chartsOpen = false;
        } else {
    ...
    gui.controls.charts.onopenFolder( openToggleCharts );
