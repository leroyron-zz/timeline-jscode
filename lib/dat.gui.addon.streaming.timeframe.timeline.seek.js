...
(function (gui, app) {
    ...
    that.goInFrame = function (exact, number) {
        _inframe(exact, number)
        that.info.text.innerHTML = 'Playback: press to play'
    }
    ...
    var _inframe = function (exact, number) {
        _timeframeSync()
        _timeframe.tick(exact, number)
    }
    var _timeframeSync = function () {
        _timeframe.duration = that.position / 100 * _timeframe.length << 0
