...
function (gui, Streaming, app) {
        ...
        var _ondrag = function (e) {
            ...
            if (that.delta == that.select) {
                if (that.select == 'nothing') {
                    ...
                    that.frame = (that.position / 100 * _timeframe.length << 0)
                    _seek.info.text.innerHTML = 'Insert: ' + that.select + ' at ' + that.frame
                }
            } else {
    ...
    gui.addon.Streaming.timeframe.timeline.seek.insert.dialog = (function (gui, app) {
        ...
        that.open = function (Authority, select, position, callback, title, description, contols) {
            ...
            time.minute = this.timeInput[0]
            time.second = this.timeInput[1]
            time.milli = this.timeInput[2]
            secMsToMinSecMsAndInput(position, time.minute, time.second, time.milli)

            attachNumberDragEvent(time.minute)
            attachNumberDragEvent(time.second)
            attachNumberDragEvent(time.milli)
            attachNumberKeyEvent(time.minute)
            attachNumberKeyEvent(time.second)
            attachNumberKeyEvent(time.milli)
        ...
        var secMsToMinSecMsAndInput = function (secMs, minute, second, milli) {
            var length = Math.floor((secMs == 0) ? 1 : Math.log10(secMs) + 1)
            minute.value = ((secMs / (100 * 60)) % 60) << 0 // minute
            ...
            milli.value = secMs.toString().substring(length - 2) // mms
            milli.value = milli.value < 0 ? '0' + milli.value : milli.value
            return secMs
        }
        ...
        var limitRenderValue = function (time) {
            var position = (time.minute.value * 6000) + (time.second.value * 100) + (time.milli.value * 1)

            if (position < _timeframe.length && position > 0) {
                // Milliseconds >
                time.milli.value = position.toString().substring(time.length - 2) // mms
                if (time.milli.value > time.milli.max - 1) {
                    time.milli.value = time.milli.min + 1
                } else if (time.milli.value < time.milli.min + 1) {
                    time.milli.value = time.milli.max - 1
                    position -= 100
                }
                ...
                // Effect Minutes >
                time.minute.value = ((position / (100 * 60)) % 60) << 0 // minute
