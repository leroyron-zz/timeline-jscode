...
(function (Streaming) {
    ...
    var evalData = function (get, duration, precision) {
        ...
        var data = new Float32Array(new ArrayBuffer(maxx * 4))
    ...
    that.eval = function (stream, instructions, relative, get, leapCallback, reassign, dispose, zeroIn, skipLeap) {
                ...
                for (let pc = 0, pclen = propChain.length; pc < pclen; pc++) {
                    ...
                    for (let p = 0, plen = props.length; p < plen; p++) {
                        ...
                        for (let s = 0; s < slen; s++) {
                            ...
                            prop[2] = Math.Type.convertToType(node[stream].conversion, prop[2])
                            buff(stream, node, prop, evalData(getset[0], deltaSetOffset, deltaSetOffset), relative, skipLeap)
                            buffOffset += deltaSetOffset
                            valueOffset += prop[2]
                        }
                    }
    ...
    that.exec = function (stream, instructions, relative, get, leapCallback, reassign, dispose, zeroIn, skipLeap) {
        _runtime.access.defaults.relative = _runtime.access.defaults.relative || false
        relative = relative || _runtime.access.defaults.relative
        var getData = []
        for (let e = 0, elen = instructions.length; e < elen; e++) {
            ...
            for (let n = 0, nlen = nodes.length; n < nlen; n++) {
                ...
                for (let ci = 0, cilen = chains.length; ci < cilen; ci++) {
                    ...
                    valueOffset += Math.Type.convertToType(node[stream].conversion, chain[3]) || 0
                    ...
                    deltaSetOffset = getset[1]
                    prop[2] = Math.Type.convertToType(node[stream].conversion, prop[1])
                    buff(stream, node, prop, evalData(getset[0], deltaSetOffset, deltaSetOffset), relative, skipLeap, true)
                    buffOffset += deltaSetOffset
                }
                // per-node
                if (leapCallback) {
    ...
    that.execLerp = function (stream, nodeSet, propSet, refnode, refprop, startVal, flux, parallel, reach, from, to, ease, exact, at, leapCallback, reassign, dispose, zeroIn, skipLeap) {
    ...
    that.assignLeap = function (stream, nodeSet, propSet, exact, at, func, reassign, dispose, zeroIn, waitForRevert) {
        var state = _runtime[stream].state
        var access = state == 'prebuff'
        ? _runtime[stream]
        : _runtime.access
        ...
        for (let n = 0, nlen = nodeSet.length; n < nlen; n++) {
            ...
            for (let p = 0, plen = propSet.length; p < plen; p++) {
                ...
                nodeBindProp.leap[dataPos] = {callback: func, dispose: typeof dispose == 'undefined' ? true : dispose, zeroIn: zeroIn, dataPosI: dataPosI}
    ...
    that.valIn = function (stream, nodeSet, propSet, val, from, to, exact, at, leapCallback, reassign, dispose, zeroIn, skipLeap) {
        var state = _runtime[stream].state
        var access = state == 'prebuff'
        ? _runtime[stream]
        : _runtime.access
            ...
            for (let n = 0, nlen = nodeSet.length; n < nlen; n++) {
                ...
                for (let p = 0, plen = propSet.length; p < plen; p++) {
                    ...
                    for (let ew = 0; ew < valDuration; ew++) {
                        ...
                        let dataPosI = data0PosI + dataPos
                        if (skipLeap) if (access.data[dataPosI + 1] == access.arguments.leap) continue
                        access.data[dataPosI + 1] = Math.Type.convertToType(nodeBind.conversion, val)
                    }
                }
            }
        }
    ...
    that.buffinData = function (stream, data, offset, nodeDataLength, propDataLength) {
        ...
        for (
            ...
            let setBind = state == 'prebuff' ? access.ids[nodeBdIK] : access.bindings.ids[nodeBdIK]
