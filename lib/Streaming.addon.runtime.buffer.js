...
(function (Streaming) {
    ...
    that.start = function () {
        ...
        let list = this.list
        while (list.length > 0) {
            let method = Array.prototype.shift.apply(list[0])// shift arguments array list[0].shift() dosen't work
            that[method].apply(this, list[0])
            list.shift()
        }
    }
    that.loaddata = function (stream, src, nodeDataLength, propDataLength, offset) {
        ...
        script.onload = function () {
            ...
            : _runtime.access
            // this.response
            if (state == 'prebuff') this.preData(stream, processData(this.response), nodeDataLength, propDataLength, offset); else access.loadData(stream, processData(this.response), nodeDataLength, propDataLength, offset)
        }
        script.send()
    }
