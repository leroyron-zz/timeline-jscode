...
(function (Streaming) {
    ...
    that.eval = function (stream, instructions, relative, precision, get, leapCallback, reassign, dispose, zeroIn, skipLeap) {
        ...
        for (let e = 0, elen = instructions.length; e < elen; e++) {
            ...
            for (let n = 0, nlen = nodes.length; n < nlen; n++) {
                ...
                // per-node
                if (leapCallback) {
                    that.assignLeap(stream, node, propSet, buffOffset, leapCallback, reassign, dispose, zeroIn)
                }
    ...
    that.assignLeap = function (stream, node, propSet, at, func, reassign, dispose, zeroIn) {
        ...
        for (let p = 0, plen = propSet.length; p < plen; p++) {
            ...
            let byteOffset = assignData.data[data0PositionI]// accuracy fix go forward (2) in offset

            let leapDataPosIDelta
            if (nodeBindProp.leap)
                if (reassign) {
                    leapDataPosIDelta = nodeBindProp.leap.dataPosI
                    assignData.data[leapDataPosIDelta] = 0
                }
            
            nodeBindProp.leap = {callback: func, dataPos: at, dispose: typeof dispose == 'undefined' ? true : dispose, zeroIn: zeroIn}
            let leapDataPos = nodeBindProp.leap.dataPos

            let dataPos = byteOffset + leapDataPos
            ...
            assignData.data[dataPosI] = assignData.arguments.leap
    ...
    var buff = function (stream, node, prop, evalData, relative, skipLeap) {
        ...
        for (let ew = 0; ew < length; ew++) {
            ...
            if (skipLeap)
                    if (assignData.data[dataPosI] == assignData.arguments.leap)
                        continue
            if (relative) {
    ...
    that.zeroOut = function (stream, from, to, node, prop, skipLeap) {
        var state = _runtime[stream].state
        var assignData = state == 'prebuff'
        ? _runtime[stream]
        : _runtime.access

        var propDataLength = assignData.propDataLength + 1// lapse
        ...
        if (node && prop) {
            ...
            for (let ew = 0; ew < zeroLength; ew++) {
                ...
                if (skipLeap)
                    if (assignData.data[dataPosI + 1] == assignData.arguments.leap)
                        continue
    ...
    that.valIn = function (stream, node, propSet, val, from, to, precision, at, leapCallback, reassign, dispose, zeroIn, skipLeap) {
        var nodeBind = node[stream]
        ...
        var precision = precision || nodeBind.precision
        ...
        for (let p = 0, plen = propSet.length; p < plen; p++) {
            ...
            let nodeBindProp = nodeBind[prop]
            let data0PositionI = nodeBindProp.data0PositionI
            let byteOffset = assignData.data[data0PositionI]
            ...
            for (let ew = 0; ew < valLength; ew++) {
                ...
                if (skipLeap)
                    if (assignData.data[dataPosI + 1] == assignData.arguments.leap)
                        continue
                assignData.data[dataPosI + 1] = val * precision
            }
        }
        if (leapCallback) {
            that.assignLeap(stream, node, propSet, at, leapCallback, reassign, dispose, zeroIn)
        }
    }

    that.getData = function (stream, node, propSet, get, skipLeap) {
        var nodeBind = node[stream]
        ...
        for (let p = 0, plen = propSet.length; p < plen; p++) {
            ...
            // byteOffset grabbed from the stream
            let byteOffset = assignData.data[data0PositionI] + 2// accuracy fix go forward (2) in offset
    ...
    that.injectDataVal = function (stream, node, prop, val, inject, skipLeap) {
        var nodeBind = node[stream]
