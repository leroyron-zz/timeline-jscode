...
(function (Streaming) {
    ...
    var buff = function (stream, node, prop, evalData, relative, skipLeap) {
        ...
        for (let ew = 0; ew < length; ew++) {
            ...
            if (skipLeap) {
                if (assignData.data[dataPosI] == assignData.arguments.leap) { continue }
            }
            if (relative) {
            ...
            for (let n = 0, nlen = nodeSet.length; n < nlen; n++) {
                ...
                for (let p = 0, plen = propSet.length; p < plen; p++) {
                    ...
                    for (let ew = 0; ew < zeroLength; ew++) {
                        ...
                        if (skipLeap) {
                            if (assignData.data[dataPosI + 1] == assignData.arguments.leap) { continue }
                        }
    ...
    that.valIn = function (stream, nodeSet, propSet, val, from, to, precision, at, leapCallback, reassign, dispose, zeroIn, skipLeap) {
        ...
        if (leapCallback) {
            that.assignLeap(stream, nodeSet[0], propSet, at, leapCallback, reassign, dispose, zeroIn)
        }
    }
