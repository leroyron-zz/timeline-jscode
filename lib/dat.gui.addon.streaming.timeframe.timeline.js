...
(function ( gui ) {
    ...
    var _onruntime = function( controller ) 
    {
        ...
        that.sliderPos = _timeframe.duration/_timeframe.length*100;
        controller.style.left = that.sliderPos+'%';

        if ((that.sliderPos-that.prevSegmentPos) > (that.segmentPos-that.prevSegmentPos))
            that.checkPassSegment();
        
        update();
    }
    var _resize = function () {
        timeline.style.width = (app.width - 395) + 'px';
    }
    ...
    //GUI
    gui.addon.onruntime(divSeek, _timeframe, _onruntime);
    ...
    var _info = gui.controls.timeline.add(that, 'info');

    gui.controls.timeline.open();

    that.sliderPos = undefined; 
    ...
    that.checkPassSegment = function() 
    {
        ...
        this.passSegment = 0;
        for (var s=0; s<slen; s++) {
          if (this.sliderPos >= segmentsPos[s]) {
            this.passSegment++;
          }
        }
    ...
    window.setInterval(function () {
      ...
      for (var a = 0; a < alen; a++) {
        if (that.sliderPos <= actionsPos[a]) {
          _actionETC.setValue(actionsPos[a] - that.sliderPos);
          break;
        } else {
