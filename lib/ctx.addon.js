...
this.ctx.Image = function (src, x, y, z, width, height, align, rotate, scale, alpha, sprite, properties, callback, args) {
    ...
    img.scale = {value: scale || 0.01, type: 'uniform'} // 0 changed 0.01 for multiplicity during buffing
...
this.ctx.loadScript = function (src, alt, callback) {
    ...
    var appendScript = function () {
        ...
        // skip to next script alternative
        script.onerror = function () {
            ...
            if (src.length > 0) {altSuccess = false; appendScript();} else if (this.batch.callback) this.batch.callback()
        }
