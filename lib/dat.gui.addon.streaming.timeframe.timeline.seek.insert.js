...
(function (gui, app) {
    ...
    that.reload = function (select, callback, ref) {
        this.load('action', this.action.init, app._fileLocal)
        this.load('sound', this.sound.init, app._fileLocal)
        this.load('comment', this.comment.init, app._fileLocal)
        this.load('segment', this.segment.init, app._fileLocal)
    }
    ...
    that.drop = function () {
        ...
        this.dialog.open(this.select + _frame + '.js', this.select, _frame, function () {
            _timeline.destroy()
            setTimeout(function () { that.reload() }, 1000)
        })
