...
(function (gui, app) {
        ...
        if (script.readyState) {  // IE
            script.onreadystatechange = function () {
                if (script.readyState == 'loaded' ||
                    ...
                    let insertTypes = ['action', 'comment', 'segment', 'sound']
                    let isPosition = true
                    if (window.Authority.main) for (let iit = 0; iit < insertTypes.length; iit++) if (window.Authority[insertTypes[iit] + 'ID'] && typeof window.Authority[insertTypes[iit] + 'ID'] != 'number') isPosition = false
                    if (isPosition) callback(window.Authority, select, position); else window.Authority.main()
    ...
    that.make = function (frame) {
        var _divElement = document.createElement('div')
        _divElement.id = frame
    ...
    that.insertAuthorities = function (data) {
        for (let di in data) {
            ...
            for (let fi in data[di]) {
                this.position = data[di][fi].position || fi / _timeframe.length * 100
